 ...
 * This file provides a concrete class for determining dates that occur in
 * between two dates.
 *
 * @package TitusCrow
...
/**
 * A temporal expression that determines if a date occurs in between (exclusive)
 * two dates.
 *
 ...
 * @license http://www.gnu.org/licenses/gpl.html GNU General Public License
 */
class TCDateInBetweenTemporalExpression implements ITCTemporalExpression {
  /**
  ...
  public function __construct(TCDate $aStartDate, TCDate $aFinishDate) {
    ...
    $this->startExpression = new TCDateAfterTemporalExpression($aStartDate);
    $this->finishExpression = new TCDateBeforeTemporalExpression($aFinishDate);
  ...
  /**
   * Evaluates if a date occurs in between the reference dates.
   *
   * @param TCDate $aDate
   * @return bool TRUE if the date occurs between (exclusive) the dates, FALSE
   * otherwise
   */
