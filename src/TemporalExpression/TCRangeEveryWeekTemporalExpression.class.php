 ...
 * @license http://www.gnu.org/licenses/gpl.html GNU General Public License
 */
class TCRangeEveryWeekTemporalExpression implements ITCTemporalExpression {
   ...
   *
   * The day of the week is a zero indexed integer starting from Sunday.
   *
   * @see TCDate
   ...
   *
   * @param TCDate $aDate
   * @return bool TRUE if the date occurs in the reference day range, FALSE
   * otherwise
   */
  public function includes(TCDate $aDate) {
      if ($this->startDayIndex > $this->finishDayIndex) {
        return (($aDate->getDayOfWeek() >= $this->startDayIndex)) ||
                (($aDate->getDayOfWeek() <= $this->finishDayIndex));
      } else {
        return (($aDate->getDayOfWeek() >= $this->startDayIndex) &&
                ($aDate->getDayOfWeek() <= $this->finishDayIndex));
      }
